...
public class Screen {
    ...
    public Image scaleImage(BufferedImage bufferedImage) {
        if (isFullScreenMode) {
            return bufferedImage.getScaledInstance(
                    fullScreenDisplayMode.getWidth(), fullScreenDisplayMode.getHeight(), Image.SCALE_FAST);
        }
    ...
    private int closestIndexOfValue(int value, int[] list) {
        ...
        for (int i = 0, min = Integer.MAX_VALUE; i < list.length; ++i) {
            ...
            final int difference = list[i] - value;
            if (difference < min) {
                min = difference;
                closestIndex = i;
            }
    ...
    private void initializeFrame() {
        ...
        fullScreenDisplayMode = findClosestDisplayMode(800, 600);
